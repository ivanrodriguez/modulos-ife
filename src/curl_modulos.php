...
# $data = '{"params":["1","1"],"method":"buscarModulosPorEstadoMunicipio","id":1}';
# ...
# $data = '{"params":["32","17"],"method":"buscarModulosPorEstadoMunicipio","id":1}';
    ...
    foreach($estado->municipios as $municipio) {
		...
		foreach($modulos_municipios->result as $modulo_municipio) {
			if(count($modulo_municipio) != 4) {
				...
				$modulo = array('numero' => $numero, 'nombre' => $nombre, 'direccion' => $direccion, 'codigo_postal' => $codigo_postal, 'referencia' => $referencia,);
				$modulos[] = $modulo;
				# var_dump($estado->id);
